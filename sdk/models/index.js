...
const bsc = require('../helpers/bsc.js');
...
const models = {
  ...
  encrypt(text, password){
    ...
    //var cipher = crypto.createCipheriv(algorithm,password,iv=IV)
  ...
  decrypt(text, password){
    ...
    //var decipher = crypto.createDecipheriv(algorithm,password,iv=IV) 
    var dec = decipher.update(text,'hex','utf8')
    dec += decipher.final('utf8');
    return dec;
  },
  //liwei
  checkPayloadSimu(req, res, next, callback) {
    const {
      contract_address
    } = req.body 
  ...
  descryptPayload1(req, res, next, callback) {
    const {
      name,
      symbol,
      total_supply,
  ...
  descryptPayload2(req, res, next, callback) {
    const {
      name,
      symbol,
      total_supply,
      ...
      models.insertToken(data, (err, response) => {
        if(err || !response) {
          ...
          async.parallel([
            ...
            (callback) => { models.processBscAccount(data, callback) } 
          ], (err, data) => {
            if(err) {
              console.log(err)
              res.status(500)
            ...
            let ethUUID = data[1]
            let bscUUID = data[2]
            console.log("bnbUUID",bnbUUID);
            console.log("ethUUID",ethUUID);
            console.log("bscUUID",bscUUID);


            models.updateTokenAddresses(uuid, bnbUUID, ethUUID, bscUUID, (err, updateResponse) => {
  ...
  validateCreateToken(body, response) {
    ...
    if(!bep20_address) {
      return 'bep20_address is required'
    }
    return true
  },
  ...
  insertToken(body, callback) {
    ...
    db.oneOrNone('insert into tokens (uuid, name, symbol, total_supply, erc20_address, bep20_address, mintable, eth_to_bnb_enabled, bnb_to_eth_enabled, bsc_to_eth_enabled, eth_to_bsc_enabled, created) values (md5(random()::text || clock_timestamp()::text)::uuid, $1, $2, $3, $4, $5, $6, true, true, true, true, now()) returning uuid', [name, symbol, total_supply, erc20_address, bep20_address, mintable])
    .then((response) => {
  ...
  processBscAccount(body, callback) {
    bsc.createAccount((err, account) => {
      if(err) {
        console.log(err)
        return callback(err)
      }

      models.insertBscAccount(account, callback)
    })
  },

  insertBscAccount(account, callback) {
    const dbPassword = bip39.generateMnemonic()
    const password = KEY+':'+dbPassword
    const aes256private = models.encrypt(account.privateKey, password)

    db.oneOrNone('insert into bsc_accounts (uuid, private_key, address, encr_key, created) values (md5(random()::text || clock_timestamp()::text)::uuid, $1, $2, $3, now()) returning uuid;', [aes256private, account.address, dbPassword])
    .then((response) => {
      callback(null, response)
    })
    .catch(callback)
  ...
  updateTokenAddresses(uuid, bnbUUID, ethUUID, bscUUID, callback) {
    db.none('update tokens set bnb_account_uuid=$1, eth_account_uuid=$2 , bsc_account_uuid=$3 where uuid=$4;', [bnbUUID.uuid, ethUUID.uuid, bscUUID.uuid, uuid])
    .then(callback)
    .catch(callback)
  },
      ...
      models.getTokenInfo(uuid, (err, tokenInfo) => {
        ...
        models.validateBalances(tokenInfo, (err, code, balanceValidation) => {
          ...
          models.getKey(tokenInfo.bnb_address, (err, key) => {
            ...
            bnb.issue(tokenInfo.name, tokenInfo.total_supply*1e8, tokenInfo.symbol, tokenInfo.mintable, key.key_name, key.password_decrypted, (err, issueResult) => {
              if(err) {
                console.log(err)
                res.status(500)
                res.body = { 'status': 500, 'success': false, 'result': err }
  ...
  validateBalances(getResponse, callback) {
    ...
    bnb.getFees((err, feesData) => {
      ...
      console.log("totalRequired BNB: ",totalRequired)
      ...
      bnb.getBalance(getResponse.bnb_address, (err, balances) => {
        ...
        if(balances.length > 0) {
          ...
          if(bal.length > 0) {
            ...
            console.log("accountBalance BNB: ",accountBalance)
          } else {
  ...
  */
  swapToken(req, res, next) {
    models.descryptPayload1(req, res, next, (data) => {
      ...
      } else if(direction === 'BinanceToEthereum') {
        models.swapBinanceToEthereum(req, res, next, data)
      } else if(direction === 'EthereumToBSC') {
        models.swapEthereumToBSC(req, res, next, data)
      } else if(direction === 'BSCToEthereum'){
        models.swapBSCToEthereum(req, res, next, data)
      }else{
        res.status(400)
        res.body = { 'status': 400, 'success': false, 'result': 'not supported yet.' }
        return next(null, req, res, next) 
      }
    })
  },
  ...
  swapEthereumToBinance(req, res, next, data) {
    ...
    models.getClientAccountForBnbAddress(bnb_address, (err, clientAccount) => {
      ...
      if(clientAccount) {
        ...
        eth.createAccount((err, account) => {
          console.log('deposit account address: ',account)
  ...
  swapEthereumToBSC(req, res, next, data) {
    const {
      bsc_address
    } = data

    models.getClientAccountForBSCAddress(bsc_address, (err, clientAccount) => {
      if(err) {
        console.log(err)
        res.status(500)
        res.body = { 'status': 500, 'success': false, 'result': err }
      ...
      if(clientAccount) {
        ...
        eth.createAccount((err, account) => {
          ...
          models.insertClientEthBscAccount(bsc_address, account, (err, clientAccount) => {
  ...
  getClientAccountForBSCAddress(bscAddress, callback) {
    db.oneOrNone('select ca.uuid, ca.bsc_address, cea.address as eth_address from client_accounts_bsc ca left join client_eth_accounts cea on cea.uuid = ca.client_eth_account_uuid where ca.bsc_address = $1;', [bscAddress])
    .then((response) => {
      callback(null, response)
    })
    .catch(callback)
  ...
  insertClientEthBscAccount(bscAddress, ethAccount, callback) {
    const dbPassword = bip39.generateMnemonic()
    const password = KEY+':'+dbPassword
    ...
    .then((returnedEthAccount) => {
      db.oneOrNone('insert into client_accounts_bsc(uuid, bsc_address, client_eth_account_uuid, created) values (md5(random()::text || clock_timestamp()::text)::uuid, $1, $2, now()) returning uuid, bsc_address;', [bscAddress, returnedEthAccount.uuid])
      .then((clientAccount) => {
        const returnObj = {
          uuid: clientAccount.uuid,
          bsc_address: clientAccount.bsc_address,
  ...
  swapBinanceToEthereum(req, res, next, data) {
    ...
    models.getClientAccountForEthAddress(eth_address, (err, clientAccount) => {
      ...
      if(clientAccount && clientAccount.bnb_address) {
        res.status(205)
        res.body = { 'status': 200, 'success': true, 'result': clientAccount }
        return next(null, req, res, next)
      } else {
        ...
        bnb.createKey(keyName, password, (err, keyData) => {
          ...
          models.insertClientBnbAccount(eth_address, keyName, password, keyData, insertone, (err, clientAccount) => {
            if(err) {
              console.log(err)
              res.status(500)
              res.body = { 'status': 500, 'success': false, 'result': err }
  ...
  swapBSCToEthereum(req, res, next, data) {
    const {
      eth_address
    } = data

    models.getClientAccountForEthAddressBSC(eth_address, (err, clientAccount) => {
      if(err) {
        console.log(err)
        res.status(500)
        res.body = { 'status': 500, 'success': false, 'result': err }
      ...
      if(clientAccount && clientAccount.bsc_address) {
        console.log(clientAccount)
        res.status(205)
        res.body = { 'status': 200, 'success': true, 'result': clientAccount }
        return next(null, req, res, next)
        ...
        bsc.createAccount((err, account) => {
          ...
          models.insertClientBscEthAccount(eth_address, insertone, account, (err, clientAccount) => {
  ...
  getClientAccountForEthAddressBSC(ethAddress, callback) {
    console.log("getClientAccountForEthAddressBSC start")
    db.oneOrNone('select ca.uuid, ca.eth_address, cba.address as bsc_address from client_accounts_eth ca left join client_bsc_accounts cba on cba.uuid = ca.client_bsc_account_uuid where ca.eth_address = $1;', [ethAddress])
    .then((response) => {
      console.log("****=> \n", response)
      callback(null, response)
    })
  ...
  insertClientBscEthAccount(ethAddress, insert, bscAccount, callback) {
    const dbPassword = bip39.generateMnemonic()
    const password = KEY+':'+dbPassword
    const aes256private = models.encrypt(bscAccount.privateKey, password)

    db.oneOrNone('insert into client_bsc_accounts(uuid, private_key, address, encr_key, created) values (md5(random()::text || clock_timestamp()::text)::uuid, $1, $2, $3, now()) returning uuid, address;', [aes256private, bscAccount.address, dbPassword])
    .then((returnedBscAccount) => {
      let sqlstr = ''
      if(insert) {
        sqlstr = 'insert into client_accounts_eth(uuid, eth_address, client_bsc_account_uuid, created) values (md5(random()::text || clock_timestamp()::text)::uuid, $1, $2, now()) returning uuid, eth_address;'
      }else {
        sqlstr = 'update client_accounts_eth set client_bsc_account_uuid=$2 where eth_address=$1 returning uuid, eth_address;' 
      }
      db.oneOrNone(sqlstr, [ethAddress, returnedBscAccount.uuid])
      .then((clientAccount) => {
        const returnObj = {
          uuid: clientAccount.uuid,
          eth_address: clientAccount.eth_address,
          bsc_address: returnedBscAccount.address
        }
        callback(null, returnObj)
      })
      .catch(callback)
  ...
  insertClientBscUpdateEthAccount(ethAddress, bscAccount, callback) {
    const dbPassword = bip39.generateMnemonic()
    const password = KEY+':'+dbPassword
    const aes256private = models.encrypt(bscAccount.privateKey, password)
    ...
    db.oneOrNone('insert into client_bsc_accounts(uuid, private_key, address, encr_key, created) values (md5(random()::text || clock_timestamp()::text)::uuid, $1, $2, $3, now()) returning uuid, address;', [aes256private, bscAccount.address, dbPassword])
    .then((returnedBscAccount) => {
      db.oneOrNone('insert into client_accounts_eth(uuid, eth_address, client_bsc_account_uuid, created) values (md5(random()::text || clock_timestamp()::text)::uuid, $1, $2, now()) returning uuid, eth_address;', [ethAddress, returnedBscAccount.uuid])
      .then((clientAccount) => {
        const returnObj = {
          uuid: clientAccount.uuid,
          eth_address: clientAccount.eth_address,
  ...
  insertClientBnbAccount(ethAddress, keyName, keyPassword, keyData, insert, callback) {
    const dbPassword = bip39.generateMnemonic()
    const password = KEY+':'+dbPassword
    const aes256seed = models.encrypt(keyData.seedPhrase, password)
    const aes256password = models.encrypt(keyPassword, password)
    ...
    .then((returnedBnbAccount) => {
      ...
      let sqlstr = ''
      if(insert) {
        sqlstr = 'insert into client_accounts_eth(uuid, eth_address, client_bnb_account_uuid, created) values (md5(random()::text || clock_timestamp()::text)::uuid, $1, $2, now()) returning uuid, eth_address;'
      }else {
        sqlstr = 'update client_accounts_eth set client_bnb_account_uuid=$2 where eth_address=$1 returning uuid, eth_address;' 
      }
      db.oneOrNone(sqlstr, [ethAddress, returnedBnbAccount.uuid])
      .then((clientAccount) => {
        const returnObj = {
          uuid: clientAccount.uuid,
          eth_address: clientAccount.eth_address,
  ...
  */
  finalizeSwap(req, res, next) {
    models.descryptPayload1(req, res, next, (data) => {
      ...
      if(direction === 'EthereumToBinance') {
        ...
        models.finalizeSwapEthereumToBSC(req, res, next, data)
      } else if(direction === 'BSCToEthereum') {
        models.finalizeSwapBSCToEthereum(req, res, next, data)
      } else{
        res.status(400)
        res.body = { 'status': 400, 'success': false, 'result': 'not supported yet' }
        return next(null, req, res, next)        
  ...
  finalizeSwapBSCToEthereum(req, res, next, data) {
    ...
    models.getClientAccountForUuidBSC2E(uuid, (err, clientAccount) => {
      if(err) {
        console.log(err)
        res.status(500)
        res.body = { 'status': 500, 'success': false, 'result': err }
      ...
      models.getTokenInfoForSwap(token_uuid, (err, tokenInfo) => {
        ...
        async.parallel([
          (callback) => { bsc.getTransactionsForAddress(tokenInfo.bep20_address, clientAccount.bsc_address, callback) },
          ...
          models.insertSwaps(newTransactions, clientAccount, token_uuid, direction, (err, newSwaps) => {
            ...
            models.proccessSwapsBSC2E(newSwaps, tokenInfo, (err, result) => {
              if(err) {
                console.log(err)
                res.status(500)
                res.body = { 'status': 500, 'success': false, 'result': err }
  ...
  finalizeSwapEthereumToBSC(req, res, next, data) {
    ...
    models.getClientAccountForUuidE2BSC(uuid, (err, clientAccount) => {
      if(err) {
        console.log(err)
        res.status(500)
        res.body = { 'status': 500, 'success': false, 'result': err }
      ...
      models.getTokenInfoForSwap(token_uuid, (err, tokenInfo) => {
        ...
        async.parallel([
          ...
          models.insertSwaps(newTransactions, clientAccount, token_uuid, direction, (err, newSwaps) => {
            ...
            models.proccessSwapsE2BSC(newSwaps, tokenInfo, (err, result) => {
              if(err) {
                console.log(err)
                res.status(500)
                res.body = { 'status': 500, 'success': false, 'result': err }
  ...
  proccessSwapsE2B(swaps, tokenInfo, callback) {
    ...
    models.getKey(tokenInfo.bnb_address, async (err, key) => {
      ...
      await bnb.initBnbClient(key.mnemonic);
      console.log("*** init sequence over\n");

      async.map(swaps, (swap, callbackInner) => {
        models.processSwapE2B(swap, tokenInfo, key, callbackInner)
      }, (err, swapResults) => {
        if(err) {
          console.log("*** aysnc maps processSwapE2B error \n", err);
          console.log("*** error swap result => \n", swapResults);
          console.log('\n')
          return callback(err)
        }
  ...
  processSwapE2B(swap, tokenInfo, key, callback) {
    bnb.transfer(key.mnemonic, swap.bnb_address, swap.amount, tokenInfo.unique_symbol, 'BNBridge Swap', (err, swapResult) => {
      if(err) {
        console.log('***bnb transfer error after transfer **\n', err)

        return models.revertUpdateWithDepositTransactionHash(swap.uuid, (revertErr) => {
          if(revertErr) {
            console.log("revertUpdateWithDepositTransactionHash error");
  ...
  proccessSwapsB2E(swaps, tokenInfo, callback) {
    ...
    models.getEthAccount(tokenInfo.eth_address, async (err, address) => {
      ...
      await eth.initNonce(tokenInfo.eth_address);
  ...
  processSwapE2BSC(swap, tokenInfo, address, callback) {
    //console.log('********** => ', n++);
    //console.log('**********\n', swap);

    bsc.sendTransaction(tokenInfo.bep20_address, address.private_key_decrypted, tokenInfo.bsc_address, swap.bsc_address, swap.amount, (err, swapResult) => {
      if(err) {
        console.log("eth send error =>\n", err)
  ...
  proccessSwapsE2BSC(swaps, tokenInfo, callback) {

    models.getBscAccount(tokenInfo.bsc_address, async (err, address) => {
      if(err || !address) {
        console.log("*** get bsc account error=>", err)
        console.log(err)
        res.status(500)
        res.body = { 'status': 500, 'success': false, 'result': 'Unable to retrieve address' }
        return next(null, req, res, next)
      ...
      await bsc.initNonce(tokenInfo.bsc_address);

      async.map(swaps, (swap, callbackInner) => {
        models.processSwapE2BSC(swap, tokenInfo, address, callbackInner)
      }, (err, swapResults) => {
        if(err) {
          console.log("processSwapE2BSC result error => \n", swapResults) 
          return callback(err)
        }
        console.log("processSwapE2BSC result => \n", swapResults)
        callback(err, swapResults)
      })
    })
  },

  proccessSwapsBSC2E(swaps, tokenInfo, callback) {
    ...
    models.getEthAccount(tokenInfo.eth_address, async (err, address) => {
      ...
      async.map(swaps, (swap, callbackInner) => {
        models.processSwapBSC2E(swap, tokenInfo, address, callbackInner)
      }, (err, swapResults) => {
        if(err) {
          return callback(err)
        }
  ...
  processSwapBSC2E(swap, tokenInfo, address, callback) {
    //console.log('********** => ', n++);
    //console.log('**********\n', swap);
  ...
  getEthAccount(ethAddress, callback) {
    ...
    .then((address) => {
      ...
      console.log("eth send address => \n", address)
      callback(null, address)
    })
    .catch(callback)
  },

  getBscAccount(bscAddress, callback) {
    db.oneOrNone('select * from bsc_accounts where address = $1;', [bscAddress])
    .then((address) => {
      if(address.encr_key) {
        const dbPassword = address.encr_key
        const password = KEY+':'+dbPassword
  ...
  revertUpdateWithDepositTransactionHash(uuid, callback) {
    console.log('*** revertUpdateWithDepositTransactionHash start');
  ...
  getClientAccountForUuidE2BSC(uuid, callback) {
    db.oneOrNone('select ca.uuid, ca.bsc_address, cea.address as eth_address from client_accounts_bsc ca left join client_eth_accounts cea on cea.uuid = ca.client_eth_account_uuid where ca.uuid = $1;', [uuid])
    .then((response) => {
      callback(null, response)
    })
    .catch(callback)
  ...
  getClientAccountForUuidBSC2E(uuid, callback) {
    db.oneOrNone('select ca.uuid, ca.eth_address, cba.address as bsc_address from client_accounts_eth ca \
    left join client_bsc_accounts cba on cba.uuid = ca.client_bsc_account_uuid where \
    ca.uuid = $1 \
    and ca.client_bsc_account_uuid is not null;', [uuid])
    .then((response) => {
      callback(null, response)
    })
    .catch(callback)
  ...
  getClientAccountForUuidB2E(uuid, callback) {
    db.oneOrNone('select ca.uuid, ca.eth_address, cba.address as bnb_address from client_accounts_eth ca \
    left join client_bnb_accounts cba on cba.uuid = ca.client_bnb_account_uuid where \
    ca.uuid = $1 \
    and ca.client_bnb_account_uuid is not null;', [uuid])
    .then((response) => {
      callback(null, response)
    })
    .catch(callback)
  ...
  getTokenInfoForSwap(tokenUuid, callback) {
    db.oneOrNone('select tok.uuid, tok.name, tok.symbol, tok.unique_symbol, tok.total_supply, tok.fee_per_swap, tok.minimum_swap_amount, tok.erc20_address, tok.bep20_address, \
    bnb.address as bnb_address, \
    bsc.address as bsc_address, \
    eth.address as eth_address from tokens tok \
    left join bnb_accounts bnb on bnb.uuid = tok.bnb_account_uuid \
    left join eth_accounts eth on eth.uuid = tok.eth_account_uuid \
    left join bsc_accounts bsc on bsc.uuid = tok.bsc_account_uuid \
    ...
    async.map(transactions,
      function (transaction, callbackInner) {
        if(direction === 'EthereumToBinance') {
          ...
          models.insertSwapE2BSC(transaction, clientAccount, tokenUuid, direction, callbackInner)
        } else if(direction === 'BSCToEthereum') {
          models.insertSwapBSC2E(transaction, clientAccount, tokenUuid, direction, callbackInner)
        } else {
          return callback("not supported yet") 
        }
      },
      function(err, result) {
        if (err) {
  ...
  insertSwapE2BSC(transaction, clientAccount, tokenUuid, direction, callback) {
    db.oneOrNone('insert into swaps (uuid, token_uuid, eth_address, bsc_address, amount, client_account_uuid, deposit_transaction_hash, direction, created) values (md5(random()::text || clock_timestamp()::text)::uuid, $1, $2, $3, $4, $5, $6, $7, now()) returning uuid, eth_address, bsc_address, amount, deposit_transaction_hash;', [tokenUuid, transaction.from, clientAccount.bsc_address, transaction.amount, clientAccount.uuid, transaction.transactionHash, direction])
    .then((response) => {
      console.log("insertSwapE2BSC response=>\n",response)
      callback(null, response)
    })
    .catch((err) => {
      callback(err)
  ...
  insertSwapBSC2E(transaction, clientAccount, tokenUuid, direction, callback) {
    console.log('insertSwapBSC2E start')
    db.oneOrNone('insert into swaps (uuid, token_uuid, eth_address, bsc_address, amount, client_account_uuid, deposit_transaction_hash, direction, created) values (md5(random()::text || clock_timestamp()::text)::uuid, $1, $2, $3, $4, $5, $6, $7, now()) returning uuid, eth_address, bsc_address, amount, deposit_transaction_hash;', [tokenUuid, clientAccount.eth_address, transaction.from, transaction.amount, clientAccount.uuid, transaction.transactionHash, direction])
    .then((response) => {
      console.log("insertSwapBSC2E response=>\n",response) 
      callback(null, response)
    })
    .catch((err) => {
      callback(err)
  ...
  insertSwapE2B(transaction, clientAccount, tokenUuid, direction, callback) {
    ...
    .then((response) => {
      console.log("insertSwapE2B response=>\n",response)
      callback(null, response)
    })
    .catch((err) => {
      callback(err)
  ...
  insertSwapB2E(transaction, clientAccount, tokenUuid, direction, callback) {
    ...
    .then((response) => {
      console.log("insertSwapB2E response=>\n",response) 
      callback(null, response)
    })
    .catch((err) => {
      callback(err)
      ...
      models.getTokenInfo(token_uuid, (err, tokenInfo) => {
        ...
        bnb.getBalance(bnb_address, (err, balances) => {
          ...
          models.getPendingBnbBalance(token_uuid, bnb_address, (err, pendingBalance) => {
            ...
            const returnObj = {
              symbol: tokenInfo.unique_symbol,
  ...
  */
  ...
  getERC20Info(req, res, next) {
    ...
    models.checkPayloadSimu(req, res, next, (data) => {
      ...
      eth.getERC20Decimals(contract_address, (err, decimals) => {
